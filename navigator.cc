...
int navigator::BFS(){
	int counter =0;
	free(discovered);path.clear();
	discovered = (int*)calloc(1,sizeof(int));
	...
	//std::queue<int, boost::circular_buffer<int>> buff{boost::circular_buffer<int>(capacity)};
	vector<int> buff;///
	buff.clear();
	bool *visited = (bool*)calloc(height*width,sizeof(bool));
	visited[map->get_start()] = true;
	//buff.push(map->get_start());
	buff.push_back(map->get_start());///
	
	int cur = 0;
	while (!buff.empty()){
		//cout<<buff.size()<<endl;
		//cout<<"|"<<buff.front()<<" cur is : "<<cur<<endl;
		cur = buff.front();
		//buff.pop();
		...
		if (cur%width-1 >= 0 && visited[cur-1] == false  ){
			if(!map->is_wall(cur%width-1)){
				buff.push_back(cur-1);
				visited[cur-1] = true;
			}
